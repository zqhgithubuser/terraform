...
# Network Acl Rule
resource "aws_network_acl_rule" "all-ingress" {
  network_acl_id = aws_network_acl.httpd.id
  rule_number    = 100
  protocol       = "-1"
  rule_action    = "allow"
...
resource "aws_network_acl_rule" "all-egress" {
  network_acl_id = aws_network_acl.httpd.id
  rule_number    = 100
  protocol       = "-1"
  rule_action    = "allow"
...
resource "aws_launch_template" "httpd" {
  ...
  depends_on = [
    aws_efs_mount_target.efs-mount-targetA,
    aws_efs_mount_target.efs-mount-targetB
  ]
}
